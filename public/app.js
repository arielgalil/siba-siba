...
function App() {
  ...
  function shuffleGroup(group) { if (!group || !Array.isArray(group.sentences) || group.sentences.length === 0) return group; const sentencesCopy = [...group.sentences]; const total = sentencesCopy.length; const result = new Array(total).fill(null); const originalMovable = []; for (let i = 0; i < total; i++) { const sentence = sentencesCopy[i]; if (sentence && sentence.movable !== undefined) { if (!sentence.movable) result[i] = sentence; else originalMovable.push(sentence); } else { console.warn("Invalid sentence:", sentence); result[i] = sentence; } } const movable = originalMovable.slice(); for (let i = movable.length - 1; i > 0; i--) { const j = Math.floor(Math.random() * (i + 1)); [movable[i], movable[j]] = [movable[j], movable[i]]; } let same = movable.length > 0 && movable.every((item, i) => item.id === originalMovable[i].id); if (same && movable.length > 1) { [movable[0], movable[1]] = [movable[1], movable[0]]; } let movableIndex = 0; for (let i = 0; i < total; i++) { if (result[i] === null) { if (movableIndex < movable.length) { result[i] = movable[movableIndex++]; } else { console.error("Shuffle error!"); const fallbackSentence = sentencesCopy.find(s => !result.includes(s)) || {id:`error-${i}`, text:"שגיאת ערבוב", movable:true}; result[i] = fallbackSentence; } } } if (result.some(item => item === null)) { console.error("Shuffle nulls!"); return group; } return { ...group, sentences: result }; }
   ...
   // --- UPDATED Functions for Sequential Difficulty Game Flow ---
   ...
   const loadNextUnplayedGroupFromLevel = (groupsInLevel, playedInLevel) => {
        const unplayed = groupsInLevel.filter((group) => !playedInLevel.has(group.originalIndex)); console.log(`Loading next unplayed. Available: ${groupsInLevel.length}, Played: ${playedInLevel.size}, Unplayed: ${unplayed.length}`);
        if (unplayed.length > 0) { const randomIndexInUnplayed = Math.floor(Math.random() * unplayed.length); const nextGroup = unplayed[randomIndexInUnplayed]; const updatedPlayedInLevel = new Set(playedInLevel).add(nextGroup.originalIndex); setPlayedInCurrentDifficulty(updatedPlayedInLevel); if (!nextGroup.sentences ) { console.error("Invalid next group structure:", nextGroup); handleReturnToSetup(); return; } const groupWithOrder = { ...nextGroup, originalOrder: nextGroup.sentences.map(s => s.id) }; setAttempts(0); setFinished(false); setCurrentGroup(shuffleGroup(groupWithOrder)); setSessionExerciseCount(prev => prev + 1); setIsLoading(false); setCheckButtonState("check"); setCheckedResults(undefined); setTimer(0); setLastCheckIncorrect(false); }
        else { console.log(`Finished level ${currentPlayingDifficulty}. Moving to next.`); const nextDifficulty = currentPlayingDifficulty + 1; if (nextDifficulty > selectedDifficultyRange.max) { handleSessionComplete(); } else { setCurrentPlayingDifficulty(nextDifficulty); loadGroupsForLevel(nextDifficulty, selectedTopics); } }
   };
   ...
   function handleReturnToSetup() { console.log("Returning to setup."); setGameState('setup'); setCurrentGroup(null); setGroupsForCurrentDifficulty([]); setPlayedInCurrentDifficulty(new Set()); setCurrentPlayingDifficulty(null); setTotalGroupsInSelection(0); setAttempts(0); setTimer(0); setScore(0); setFinished(false); setCheckButtonState('check'); setCheckedResults(undefined); setLastCheckIncorrect(false); setSessionExerciseCount(0); } // Reset score on return to setup
   function renderSentence(sentence, index) { const baseClasses = [ 'my-1', 'w-full', 'max-w-md', 'rounded-xl', 'relative', 'flex', 'items-center', 'p-3', 'transition-colors', 'duration-300', 'shadow-sm', 'border' ]; let stateClasses = 'bg-white dark:bg-gray-700 border-gray-300 dark:border-gray-600 text-gray-900 dark:text-gray-100'; let cursorClass = 'cursor-grab'; if (checkedResults !== undefined && checkedResults[index] !== null) { if(checkedResults[index]) { stateClasses = 'bg-green-100 dark:bg-green-900 border-green-400 dark:border-green-600 text-green-800 dark:text-green-200'; cursorClass = 'cursor-default'; } else { stateClasses = 'bg-red-100 dark:bg-red-900 border-red-400 dark:border-red-600 text-red-800 dark:text-red-200'; } } let fixedSpecificClasses = ''; if (sentence && !sentence.movable) { fixedSpecificClasses = 'fixed border-dashed border-gray-400 dark:border-gray-500'; cursorClass = 'cursor-default'; } const iconText = sentence.movable ? "↕️" : "🔒"; return createElement( 'div', { key: sentence.id, 'data-id': sentence.id, className: [...baseClasses, stateClasses, cursorClass, fixedSpecificClasses, (!sentence.movable ? '' : 'hover:shadow-md'), 'sentence-box'].join(' ') }, createElement('span', { className: 'icon ml-2 text-xl flex-shrink-0 cursor-default text-gray-500 dark:text-gray-400' }, iconText), createElement('span', { className: 'sentence-text flex-grow break-words select-text text-sm sm:text-base' }, sentence.text) ); }
...
const GameHeader = () => {
    ...
    return createElement(React.Fragment, null, topRow, topicSubtitle, statusBar, instructions, finishMessage );
};
// ----------------------------------

  // Action Button Logic
const getButtonClasses = () => {
    // *** שינוי: הקטנת padding וגודל טקסט ב-base ***
    let base = 'mt-4 py-1.5 px-5 text-base rounded-full font-semibold transition-all duration-300 focus:outline-none focus:ring-2 focus:ring-offset-2 dark:focus:ring-offset-gray-800'; // הקטנו padding וגודל טקסט
    let stateClasses = '';
  ...
  let buttonText; let buttonOnClick; let buttonDisabled = isLoading;
  if (checkButtonState === "session_finished") { buttonText = "חזרה לבחירת תרגילים"; buttonOnClick = handleReturnToSetup; buttonDisabled = false; }
  else if (checkButtonState === "ready") { buttonText = "מוכן לאתגר הבא?"; buttonOnClick = nextLevel; buttonDisabled = isLoading; }
  else if (checkButtonState === "checking") { buttonText = "בודק..."; buttonOnClick = () => {}; buttonDisabled = true; }
  else { if (lastCheckIncorrect) { buttonText = "נסה שוב!"; } else { buttonText = "בדיקה"; } buttonOnClick = checkOrder; buttonDisabled = isLoading || (!currentGroup && gameState === 'playing'); } // Disable if loading or no group in playing state
  const actionButton = gameState === 'playing' ? createElement( 'button', { className: getButtonClasses(), onClick: buttonOnClick, disabled: buttonDisabled }, buttonText ) : null;

  // --- Main Return (עם הצגת Header תמיד) ---
  return createElement(
    'div', { className: 'container flex flex-col items-center justify-start pt-2 pb-6 min-h-screen gap-3 px-2 sm:px-4 relative' },
    createElement(GameHeader, null), // כותרת מוצגת תמיד
    ...
    gameState === 'setup'
      ...
      : // מסך משחק
        createElement(React.Fragment, null,
            createElement( 'div', { id: 'sortable-container', ref: containerRef, className: `flex flex-col items-center w-full max-w-md ${isLoading || finished ? 'min-h-[200px]' : ''}` },
                ...
                : null // הודעת סיום מוצגת ב-GameHeader
            ),
            actionButton
        ),
    ToastComponent
