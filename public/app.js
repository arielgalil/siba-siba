...
function App() {
    ...
    const handleStartGame = (difficultyRange, topics, totalCount) => {
        ...
        setSelectedTopics(topics); setTotalGroupsInSelection(totalCount);
    ...
    const loadNextUnplayedGroupFromLevel = (groupsInLevel, playedInLevel) => {
        ...
        if (unplayed.length > 0) {
            ...
            if (nextDifficulty > selectedDifficultyRange.max) {
                ...
                setFinished(true); setIsFetchingGroups(false); setCurrentGroup(null);
            } else {
    ...
    function GameSetup({ initialDifficultyRange, initialTopics, onStartGame }) {
         ...
         const [topicCounts, setTopicCounts] = useState({}); const [countsLoading, setCountsLoading] = useState(false);
         const [totalAvailableInSelection, setTotalAvailableInSelection] = useState(0);
         ...
         useEffect(() => { /* Fetch counts when difficulty changes */
             const fetchCounts = async () => {
                 ...
                 } catch (error) { console.error("Error fetching topic counts:", error); setTopicCounts({}); setToast({ show: true, message: 'שגיאה בטעינת מספרי תרגילים', type: 'error' }); } finally { setCountsLoading(false); }
             };
         ...
         // *** Updated renderSelectionButton for centered text ***
         const renderSelectionButton = ({ type, id, name, value, checked, onChange, labelText, count }) => {
            // Base classes - removed text-right, keep justify-between for layout
            ...
            // Input needs margin to space from centered text (ml-3 = margin-left for RTL)
            ...
            return createElement('label', { htmlFor: id, className: `${labelBaseClasses} ${checked ? labelSelectedClasses : labelUnselectedClasses}` },
                 // Text container - Added items-center and text-center
                 createElement('div', { className: 'flex flex-col items-center text-center flex-grow' }, // Added flex-grow to help centering with justify-between
                      ...
                      createElement('span', { className: 'text-xs text-gray-500 dark:text-gray-400 mt-1' }, `[${count} ${count === 1 ? 'תרגיל' : 'תרגילים'}]`)
                 ),
                 // Input remains at the end visually in RTL due to justify-between and its own margin
                 createElement('input', { type: type, id: id, name: name, value: value, checked: checked, onChange: onChange, className: inputClasses })
            );
         };
         // *** End Updated renderSelectionButton ***
    ...
    const getButtonClasses = () => {
        ...
        // Use isFetchingGroups instead of isLoading for button disable during game loading
        if (isFetchingGroups || (!currentGroup && gameState === 'playing' && !finished)) { stateClasses = 'bg-gray-400 text-gray-700 opacity-50 cursor-not-allowed dark:bg-gray-600 dark:text-gray-400'; }
        return `${base} ${stateClasses}`;
    };

    let buttonText = '';
    let buttonOnClick = () => {};
    let buttonDisabled = isLoading; // Use initial isLoading here, getButtonClasses handles isFetchingGroups
    if (gameState === 'playing' && !finished) {
        if (checkButtonState === "ready") {
            buttonText = "מוכן לאתגר הבא?";
            buttonOnClick = nextLevel; buttonDisabled = isFetchingGroups; // Disable on fetch
        } else if (checkButtonState === "checking") {
            buttonText = "בודק...";
            buttonOnClick = () => {}; buttonDisabled = true;
        } else { // check state
            if (lastCheckIncorrect) { buttonText = "הסדר לא נכון - נסה שוב!"; }
            else { buttonText = "בדיקה"; }
            buttonOnClick = checkOrder; buttonDisabled = isFetchingGroups || !currentGroup; // Disable on fetch or no group
        }
    }
    ...
    return createElement(
        ...
        createElement('div', { className: 'w-full max-w-lg mx-auto p-4 sm:p-6 bg-white dark:bg-gray-800 shadow-md rounded-lg flex flex-col items-center' },
            ...
            gameState === 'playing' && !finished ?
            createElement(React.Fragment, null, /* Game Area Elements */
                 // *** Updated Game Title H2 ***
                 createElement('h2', { className: 'text-lg sm:text-xl font-semibold text-center mb-1 text-gray-800 dark:text-gray-200 break-words' }, // Added break-words for wrapping
                      currentGroup ?
                      `${currentGroup.title || 'תרגיל'} | ${currentGroup.topic || 'כללי'} | ${getDifficultyString(currentGroup.difficulty)}`
                      : (isFetchingGroups ? 'טוען תרגילים...' : '')
                 ),
                 // *** End Updated Game Title H2 ***
